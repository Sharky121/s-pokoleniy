...
class VeteransTableSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        $connection->transaction(function () use ($connection) {
            ...
            $i = 1;
            $j = 1;
            foreach ($connection->select('select * from articles where category = ?', ['veterans']) as $record) {
                $connection->query()->from('veterans')->insert([
                    'id' => $i,
                ...
                foreach ($images as $k => $image) {
                    ...
                    $connection->query()->from('veterans_photos')->insert([
                        'id' => $j,
