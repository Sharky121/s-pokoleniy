...
class ArtPhotosController extends Controller
{
    public function batchStore(Request $request, Art $parent)
    {
        ...
        if ($result->succeeded()) {
            ...
            return response()
                ->json(['redirectTo' => route('admin.art.edit', [$parent], false)]);
        }
    ...
    public function batchUpdate(Request $request, Art $parent)
    {
        $connection = $parent->getConnection();
        $query = $parent->photos();
    ...
    public function destroy(Request $request, Art $parent, ArtPhoto $model)
    {
