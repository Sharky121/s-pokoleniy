...
use App\Models\PublishingPhoto;
...
class PublishingsPhotosController extends Controller
{
    public function batchStore(Request $request, Publishing $parent)
    {
        ...
        if ($result->succeeded()) {
            ...
            return response()
                ->json(['redirectTo' => route('admin.publishings.edit', [$parent], false)]);
        }
    ...
    public function batchUpdate(Request $request, Publishing $parent)
    {
        $connection = $parent->getConnection();
        $query = $parent->photos();
    ...
    public function destroy(Request $request, Publishing $parent, PublishingPhoto $model)
    {
